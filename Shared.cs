    ...
    public class Shared
    {
        ...
        public static bool CheckSpawnRegionBanned(string GUID)
        {
            ...
            if (MyMod.BannedSpawnRegions.TryGetValue(GUID, out CanBeUnbannedIn))
            {
                ...
                {
                    // We need seed for Random, because THAT method can be called in same tick few times,
                    // that will lead to same result. Because by default random seed generated by system time.
                    int Seed = new Guid().GetHashCode();

                    // We do not use UnityEngine.Random, because Shared.cs is used also on Dedicated server
                    // That DOES NOT HAVE Unitys stuff at all.
                    System.Random RNG = new System.Random(Seed);

                    if (RNG.Next(0, 100) <= 23) // with 23% chance ban this spawner, instead of spawning animal.
                    {
                        BanSpawnRegion(GUID);
                        return true;
                    }
